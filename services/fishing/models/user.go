...
type User struct {
	...
	NickName        string `xorm:"not null default '' VARCHAR(256) comment('昵称')" json:"nick_name"`
	...
	RegisterTime    int64  `xorm:"not null default 0 INT(11) comment('创建时间')" json:"register_time"`
	...
	CoinTotal       int64  `xorm:"not null default 0 INT(11) comment('收入汇总')" json:"coin_total"`
	CoinPayTotal    int64  `xorm:"not null default 0 INT(11) comment('购买汇总')" json:"coin_pay_total"`
	CoinRefillTotal int64  `xorm:"not null default 0 INT(11) comment('充值汇总')" json:"coin_refill_total"`
	UserStatus      int64  `xorm:"not null default 0 INT(11) comment('用户状态 0 正常 1 拉黑')" json:"user_status"`
	UserType        int64  `xorm:"not null default 0 INT(11) comment('用户类型 0 用户 1 admin')" json:"user_type"`
	Motto           string `xorm:"not null default '' VARCHAR(256) comment('座右铭')" json:"motto"`
}
...
func UpdateUserAffected(filter mysql.OrmFilter, message *User) (affected int64, err error) {
	session := mysql.GetDB().NewSession()
	defer session.Close()
	if filter != nil {
		session = filter(session)
	}
	affected, err = session.Update(message)
	return
}
