...
func (m *MobileFishingService) ListActiclesByTag(ctx context.Context, request *mobile_fishing.ListActiclesByTagRequest, response *mobile_fishing.ListActiclesByTagResponse) (err error) {
	response.Status = &mobile_fishing.Status{}
	logger := log.WithFields(log.Fields{
		"Module": "Service",
		"Method": "ListActiclesByTag",
	})

	if len(request.ListTagIds) < 1 || request.PageSize < 1 {
	...
	ListArticles, err := models.ListArticles(func(session *xorm.Session) *xorm.Session {
		session = session.Table("article_tag_detail as d").
			Join("left", "article as a", "d.article_id = a.id").
			In("d.article_tag_id", request.ListTagIds)
		if request.IfNew {
			session = session.OrderBy("a.create_time desc")
		} else {
			session = session.OrderBy("a.view_count desc")
		}
		return session.Where("article_status = ?", mobile_fishing.ActicleStatus_acticle_status_author_pass).Select("a.*").Where("is_del = ?", 0).
