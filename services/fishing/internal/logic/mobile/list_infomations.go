...
func (m *MobileFishingService) ListInfomations(ctx context.Context, request *mobile_fishing.ListInfomationsRequest, response *mobile_fishing.ListInfomationsResponse) (err error) {
	response.Status = &mobile_fishing.Status{}
	logger := log.WithFields(log.Fields{
		"Module": "Service",
		"Method": "ListInfomations",
	})
	...
	ListInfomations, err := models.ListInfomations(func(session *xorm.Session) *xorm.Session {
		if request.City != "" {
			session = session.Where("city = ?", request.City)
		}
		if request.Province != "" {
			session = session.Where("province = ?", request.Province)
		}
		return session.Where("infomation_status = ?", mobile_fishing.InfomationStatus_infomation_status_author_pass).Where("is_del = ?", 0).OrderBy("create_time desc").Limit(int(pageSize), int((page-1)*pageSize))
	})
	if err != nil {
	...
	for i := 0; i < len(ListInfomations); i++ {
		if v, ok := ListUsersMap[ListInfomations[i].UserId]; ok {
			infomation := &mobile_fishing.Infomation{
				...
				InfomationCategoryType: request.InfomationCategoryType,
			}
