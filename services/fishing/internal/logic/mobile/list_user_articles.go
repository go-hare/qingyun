...
func (m *MobileFishingService) ListUserArticles(ctx context.Context, request *mobile_fishing.ListUserArticlesRequest, response *mobile_fishing.ListUserArticlesResponse) (err error) {
	response.Status = &mobile_fishing.Status{}
	logger := log.WithFields(log.Fields{
		"Module": "Service",
		"Method": "ListUserArticles",
	})

	if request.UserId < 1 || request.PageSize < 1 || request.ReleaseUserId < 1 {
	...
	ListArticles, err := models.ListArticles(func(session *xorm.Session) *xorm.Session {
		return session.Where("user_id = ?", request.ReleaseUserId).
